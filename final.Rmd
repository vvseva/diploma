...
many_models_authors <- data_frame(K = c(20, 30, 40)) %>%
...
```{r}
DS = c("gini", "loss", "sensitivity analysis*", "factorial design", "process model", "microsimulation") %>% as_tibble()

Rules = c("decision tree*", "decision process*", "dummy", "synthetic", "individual actor*", "dynamics model*", "emerg*")%>% as_tibble()

Networks = c("social network*", "odd", "peer effect*") %>% as_tibble()

Theory = c("bias*", "planned behav*", "social norm*", "risk aversion", "prospect*", "payoff*", "reward*", "hueristic*", "complex*", "cooperat*", "social system*", "social interaction*", "opinion dynamics", "conceptual model", "opinion formation", "social impact", "multi-agent", "social norm") %>% as_tibble()
...
dictionary_max[["δrules"]] = c("decision rule*", "simple rule*", "set of rules", "behavior rule*", "working memory", "wm", "ifelse")
dictionary_max[["δbdi"]] = c("bdi", "desire*", "belief*", "intention*")
dictionary_max[["δmachinelearning"]]  =c("machine learning", "probabilistic model*", "regression*", "data mining", "probanilit*", "logistic", "time series", "fuzzy")
dictionary_max[["δgametheory"]] = c("game theory", "dilemma")
dictionary_max[["δboundedrationality"]] = c("bounded*", "satisficing")
...
```{r}
kwic(corpus_max, pattern = phrase("spatial")
...
texts_words_long %>% 
  ...
  group_by(topic, words) %>% 
  mutate(x.y = value) %>% 
  mutate(G2 = logDice(x.y, x.topic, y.words)) %>% 
  mutate(G2 = ifelse(G2 < 0, 0, G2)) %>% 
  arrange(-G2)
  select(topic, words, G2) %>% 
  pivot_wider(names_from = topic, values_from = G2) %>% 
  ungroup() %>% 
  rename(words=1, topic1 = 2, topic2 = 3) %>% 
  mutate(dif = (topic1- topic2)) %>% 
  mutate(bigger = ifelse(topic1 - topic2 >= 0, "cyan4", "magenta4")) %>% 
  filter(dif > 0.5 | dif < -0.5) %>% 
  mutate(words = fct_reorder(words, dif)) %>%  
  ggplot(aes(x = topic1, xend = topic2, y = words, color = bigger))+
          geom_dumbbell(aes(color = bigger), size_x = 3.5, 
                      size_xend = 3.5,colour_x="cyan4", 
                      colour_xend = "magenta4")+
  theme_minimal()+
  ggtitle("Keywords between topics 6 and 18", subtitle = "G2 metrics")+
