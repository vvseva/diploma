...
```{r}
DS = c("probabilistic model*", "regression*", "machine learning", "data mining", "probanilit*", "logistic", "gini", "loss", "sensitivity analysis*", "factorial design", "process model", "microsimulation", "time series") %>% as_tibble()
...
Theory = c("bounded*", "bias*", "planned behav*", "satisficing", "desire*", "belief*" ,"social norm*", "risk aversion", "prospect*", "payoff*", "game theory", "reward*", "bdi", "hueristic*", "complex*", "complexit*", "cooperat*", "dilemma", "bounded confid*", "complex system*", "social system*", "social interaction*", "opinion dynamics", "conceptual model", "synthetic population", "opinion formation", "social impact", "multi-agent", "social norm") %>% as_tibble()
...
dictionary_max[["optimization"]] = c("optimization", "minimization", "maximization", "optimiz*", "minimiz*", "maximiz*")
dictionary_max[["networks"]] = c("network*", "edge*", "node*", "centrality")
dictionary_max[["barabasi"]] = c("barabasi", "scale free", "ba network", "random betwork", "preferential attachment")
dictionary_max[["watts"]] = c("small world", "watts", "small-world")
```
...
```{r}
kwic(corpus_max, pattern = phrase("degree dependence*")
...
words_long_c %>% 
  filter(doi %in% (stm_topic_doi %>% filter(topic == topic_to_look) %>% select(doi) %>% pull()) ) %>% select(doi) %>% unique() %>% count()
